...
class _Matcher(object):
    ...
    def __init__(self, which, entry):
        _assert(0 <= which <= YEAR_OFFSET, "improper number of cron entries specified")
    ...
    def _parse_crontab(self, which, entry):
        ...
        def _fix(it):
            ...
            it = int(it, 10)
            _assert(_start <= it <= _end_limit, "item value %r out of range [%r, %r]", it, _start, _end_limit)
            return it
        ...
        def _parse_piece(it):
            ...
            _assert(start <= end, "%s range start value %r > end value %r", _attribute[which], start, end)
        ...
        if entry in ("*", "?"):
            if entry == "?":
                _assert(which in (DAY_OFFSET, WEEK_OFFSET), "cannot use '?' in the %r field", _attribute[which])
            return None, _end

        # last day of the month
        if entry == "l":
            _assert(which == DAY_OFFSET, "you can only specify a bare 'L' in the 'day' field")
            return None, _end

        # for the last 'friday' of the month, for example
        elif entry.startswith("l"):
            _assert(which == WEEK_OFFSET, "you can only specify a leading 'L' in the 'weekday' field")
            es, _, ee = entry[1:].partition("-")
            _assert(
                (entry[1:].isdigit() and 0 <= int(es) <= 7)
                or (_ and es.isdigit() and ee.isdigit() and 0 <= int(es) <= 7 and 0 <= int(ee) <= 7),
        ...
        if "/" in entry:
            ...
            _assert(
                increment <= _end_limit, "increment value must be less than %r, you provided %r", _end_limit, increment
            )
...
class CronTab(object):
    ...
    def _make_matchers(self, crontab):
        ...
        if len(ct) == 5:
            ...
            ct.insert(0, "0")
        _assert(len(ct) == 7, "improper number of cron entries specified; got %i need 5 to 7" % (len(ct,)))
